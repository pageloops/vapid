...
async function _content(ctx) {
  ...
  for (const field of allowedFields) {
    ...
    fields[field] = ctx.state.section.fields[field];
  }
  ...
  for (const file of ctx.request.files) {
    ...
    if (allowedFields.has(fieldName)) {
      let multiple = fields[fieldName].type == 'picture';
      promises.push(_saveFile(file, multiple).then((c) => { content[fieldName] = c; }));
      console.log('save file. field', fieldName, 'multiple', multiple, 'field', fields[fieldName]);
    }
  }
...
async function _saveFile(file, multiple = false) {
  const buffer = readFileSync(file.path);
  const ext = extname(file.path);
  const digest = _fileDigest(file);

  // Sharp can't output SVG or animated GIF
  if (animated(buffer) || ext  === '.svg') {
  ...
  if (!multiple) {
    return await saveOriginalFormat(buffer, digest, ext);
  }

  let files = [];
  files.push(await saveWebpCopy(buffer, digest));
  files.push(await saveOriginalFormat(buffer, digest, ext));
  return files;
}

async function saveWebpCopy(buffer, digest) {
...
async function saveOriginalFormat(buffer, digest, ext) {
...
function _fileDigest(file) {
  return Utils.checksum(file.path);
}
