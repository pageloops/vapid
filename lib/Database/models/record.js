...
module.exports = (sequelize, DataTypes) => {
  ...
  const Record = sequelize.define('Record', {
    ...
    getterMethods: {
      ...
      permalink() {
        ...
        const name = Utils.kebabCase(this.content.title || this.content.name || 'perm');
  ...
  Record.prototype.previewContent = function previewContent(fieldName, section) {
    const MAX_LEGNTH = 140;
    const directive = directives.find(section.fields[fieldName]);

    if (directive.type === 'TextDirective' || directive.type === 'HTMLDirective') {
      const content = directive.preview(this.content[fieldName]);
      if (content.length > MAX_LEGNTH) {
        return content.slice(0,MAX_LEGNTH-3) + '...';
      }
    }

    return directive.preview(this.content[fieldName]);
  };
